...
$(function(){
    ...
    createStartTime.bind("focus",function(){
        validateTip(createStartTime.next(),{"color":"#666666"},"* 点击输入框，选择日期",false);
    }).bind("blur",function(){
        if(createStartTime.val() != null && createStartTime.val() != ""){
            validateTip(createStartTime.next(),{"color":"green"},imgYes,true);
        }else{
            validateTip(createStartTime.next(),{"color":"red"},imgNo + " 选择的日期不正确,请重新输入",false);
        }
    });
